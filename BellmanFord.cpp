...
void BellmanFord::relaxEdges(std::list<Edge<int,int,int> > edgeList, int size) {
  for (int i = 1; i < size; i++) {
    for (std::list<Edge<int,int,int> >::iterator it = edgeList.begin();
	 it != edgeList.end(); ++it) {
      Edge<int,int,int> e = *it;
      if ((distance_[e.source_] + e.weight_) < distance_[e.sink_]) {
	distance_[e.sink_] = (distance_[e.source_] + e.weight_);
...
bool BellmanFord::hasNegativeCycle(std::list<Edge<int,int,int> > edgeList) {
  for (std::list<Edge<int,int,int> >::iterator it = edgeList.begin();
       it != edgeList.end(); ++it) {
    Edge<int,int,int> e = *it;
    if ((distance_[e.source_] + e.weight_) < distance_[e.sink_])
